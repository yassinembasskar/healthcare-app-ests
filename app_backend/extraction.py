...
def getmessage(imagefile):
    ...
    # Preprocess image (grayscale, thresholding, noise removal, etc.)
    gray = get_grayscale(image)
    thresh = thresholding(gray)
    opened = opening(gray)
    cannied = canny(gray)
    images = {'gray': gray, 'thresh': thresh, 'opening': opened, 'canny': cannied}

    # Use PaddleOCR instead of Tesseract
    ocr = PaddleOCR(use_angle_cls=True, lang='en')  # Initialize PaddleOCR
    result = ocr.ocr(imagefile, cls=True)  # Perform OCR

    # Extract the OCR output text
    message = ''
    for line in result[0]:  # Each line contains [ [x, y, z, ...] , text ]
        message += line[1] + ' '  # Concatenate the detected text
